...
def linear_search(arr: List, item: int) -> Optional[int]:
...
def linear_search_gen(arr: List, item: int) -> Optional[int]:
...
def linear_recursive(arr: List, item: int) -> Optional[int]:
	return recursive_iteration(arr, item, 0, len(arr) - 1)

def linear_recursive_gen(arr: List, item: int) -> int:
	i = -1
	while i < len(arr):
