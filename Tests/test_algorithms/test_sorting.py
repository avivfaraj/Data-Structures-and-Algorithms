...
def test_sorting(num: int, function ) -> None:
   ...
   assert is_sorted(arr) == True, f"num: {num}, function: '{function.__name__}' had a problem"
...
def test_radix_sort(num: int ) -> None:
   ...
   # Testing quick and merge sort
   assert is_sorted(radix_sort(arr)) == True, f"Radix Sort had a problem with num: {num}"
